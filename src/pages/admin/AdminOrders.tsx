...
const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data, error } = await api.get<any[]>('/admin/orders');
  ...
  const fetchOrderItems = async (orderId: string) => {
    try {
      const { data, error } = await api.get<any>(`/admin/orders/${orderId}`);
      if (error || !data) throw new Error(error || 'Failed to fetch order items');
  ...
  const updateOrderStatus = async (orderId: string, status: string) => {
    try {
      const { error } = await api.put(`/admin/orders/${orderId}/status`, { status });
      if (error) throw new Error(error || 'Failed to update order status');
