...
const WishlistPage = () => {
  ...
  const fetchWishlist = async () => {
    try {
      const { data, error } = await api.get<any[]>('/wishlist');
      if (error || !data) throw new Error(error || 'Failed to fetch wishlist');
      setItems(data);
    } catch (error) {
  ...
  const removeFromWishlist = async (productId: string) => {
    try {
      ...
      if (error) throw new Error(error || 'Failed to remove from wishlist');
