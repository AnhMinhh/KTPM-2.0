...
import { ArrowLeft, Package, Calendar, DollarSign, Truck, Check } from 'lucide-react';
...
const OrderDetailPage = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        const { data, error } = await api.get<any>(`/orders/${orderId}`);
        if (error || !data) throw new Error(error || 'Failed to fetch order');
        setOrder(data.order);
        setItems(data.items || []);
      } catch (error) {
