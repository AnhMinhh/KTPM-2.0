...
const RecentlyViewedPage = () => {
  ...
  const fetchRecentlyViewed = async () => {
    try {
      const { data, error } = await api.get<any[]>('/viewed');
      if (error || !data) throw new Error(error || 'Failed to fetch recently viewed');
      setItems(data);
    } catch (error) {
  ...
  const clearHistory = async () => {
    try {
      const { error } = await api.delete('/viewed');
      if (error) throw new Error(error || 'Failed to clear history');
      setItems([]);
