...
-- =============================================
-- 15. CREATE Permissions TABLE (theo báo cáo)
-- =============================================
...
BEGIN
    CREATE TABLE Permissions (
        ...
        permission_role_id NVARCHAR(450),
        permission_title NVARCHAR(256) NOT NULL,
        permission_module NVARCHAR(256) NOT NULL,
        permission_description NVARCHAR(MAX),
        CreatedAt DATETIME2 DEFAULT GETUTCDATE(),
        FOREIGN KEY (permission_role_id) REFERENCES AspNetRoles(Id) ON DELETE SET NULL
    );
    
    CREATE INDEX IX_Permissions_Module ON Permissions(permission_module);
    CREATE INDEX IX_Permissions_RoleId ON Permissions(permission_role_id);
END
GO

-- =============================================
-- 16. CREATE RolePermissions TABLE (Ánh xạ Role-Permission)
-- =============================================
...
-- =============================================
-- 17. CREATE Payments TABLE (theo báo cáo - Payment Class)
-- =============================================
...
BEGIN
    CREATE TABLE Payments (
        ...
        Shopping_id INT,
        Shopping_name NVARCHAR(256),
        Shopping_description NVARCHAR(MAX),
...
-- =============================================
-- 18. CREATE Reviews TABLE (Đánh giá sản phẩm)
-- =============================================
...
-- =============================================
-- 19. CREATE ShoppingCart TABLE (Giỏ hàng - từ báo cáo)
-- =============================================
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'ShoppingCarts')
BEGIN
    CREATE TABLE ShoppingCarts (
        ...
        shopping_name NVARCHAR(256),
        shopping_description NVARCHAR(MAX),
        shopping_cart_type NVARCHAR(100),
        UserId NVARCHAR(450) NOT NULL,
        CreatedAt DATETIME2 DEFAULT GETUTCDATE(),
        UpdatedAt DATETIME2 DEFAULT GETUTCDATE(),
        FOREIGN KEY (UserId) REFERENCES AspNetUsers(Id) ON DELETE CASCADE
...
-- =============================================
-- 20. CREATE CartItems TABLE (Chi tiết giỏ hàng)
-- =============================================
...
PRINT '  - ViewedEntries';
PRINT '  - Permissions (New)';
PRINT '  - RolePermissions (New)';
PRINT '  - Payments (New)';
PRINT '  - Reviews (New)';
PRINT '  - ShoppingCarts (New)';
PRINT '  - CartItems (New)';
PRINT '=============================================';
GO
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (
            @AdminId, 'admin@example.com', 'ADMIN@EXAMPLE.COM', 'admin@example.com', 'ADMIN@EXAMPLE.COM',
            1, 'AQAAAAIAAYagAAAAEPzz5q8X0Z5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ5qZ=', 
            NEWID(), NEWID(), NULL, 0, 0, 1, 0
        );
        ...
        INSERT INTO Profiles (UserId, Email, Username, FullName, CreatedAt, UpdatedAt)
        VALUES (@AdminId, 'admin@example.com', 'admin', 'Administrator', GETUTCDATE(), GETUTCDATE());
    END
END
GO

-- =============================================
-- INSERT PERMISSIONS (Quyền hạn theo DFD)
-- =============================================
DELETE FROM RolePermissions WHERE PermissionId IN (SELECT Id FROM Permissions WHERE permission_id IS NOT NULL OR permission_id IS NULL);
DELETE FROM Permissions;

INSERT INTO Permissions (Id, permission_id, permission_title, permission_module, permission_description)
VALUES 
    (NEWID(), 1, 'Manage Shopping', 'Shopping', 'Quyền quản lý mua sắm'),
    (NEWID(), 2, 'Manage Shopping Cart', 'Shopping', 'Quyền quản lý giỏ hàng'),
    (NEWID(), 3, 'Manage Order', 'Order', 'Quyền quản lý đơn hàng'),
    (NEWID(), 4, 'Manage Payment', 'Payment', 'Quyền quản lý thanh toán'),
    (NEWID(), 5, 'Manage Product', 'Product', 'Quyền quản lý sản phẩm'),
    (NEWID(), 6, 'Manage Category', 'Category', 'Quyền quản lý danh mục'),
    (NEWID(), 7, 'Manage User', 'User', 'Quyền quản lý người dùng'),
    (NEWID(), 8, 'View Report', 'Report', 'Quyền xem báo cáo'),
    (NEWID(), 9, 'Search Product', 'Product', 'Quyền tìm kiếm sản phẩm'),
    (NEWID(), 10, 'View Account', 'Account', 'Quyền xem tài khoản'),
    (NEWID(), 11, 'Edit Account', 'Account', 'Quyền chỉnh sửa tài khoản');
GO

-- =============================================
-- INSERT ROLE PERMISSIONS (Gán quyền cho Admin Role)
-- =============================================
IF NOT EXISTS (SELECT 1 FROM RolePermissions WHERE RoleId IN (SELECT Id FROM AspNetRoles WHERE NormalizedName = 'ADMIN'))
BEGIN
